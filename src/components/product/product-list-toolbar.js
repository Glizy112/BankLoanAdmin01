...
import { getAllTasks, update } from "../../utils/FirebaseController";
  ...
  const handleSubmit = () => {
    ...
    const filteredData = arr.filter(
      (value, index, self) =>
        ...
        self.findIndex(
          (t) =>
            t?.["Application Id"].toString() === value?.["Application Id"].toString() &&
            t?.["Customer Name"].toString() === value?.["Customer Name"].toString()
        )
    );

    const results = filteredData.filter(
      ({ ["Application Id"]: id1, ['Type Of Applicant']: id11 }) =>
        !newArr?.some(
          ({ ["Application Id"]: id2, ['Type Of Applicant']: id22 }) =>
            String(id2) === String(id1) && String(id11) === String(id22)
        )
    );
    if (results.length > 0) {
      results?.map((item) => {
        if (!item?.["Type Of Applicant"]) {
          setMsg("Type of Applicant is required!");
          setAlertOpen(true);
        } else {
